...
export default function EditServicePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (serviceRes.data.success) {
          ...
          // Ensure the provider owns this service
          if (s.provider?._id !== user._id && s.provider !== user._id) {
  ...
  if (authLoading || isFetching) return <div className="p-10 text-center">Loading service details...</div>;
    ...
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted mt-1">Update your existing service listing.</p>
        </div>
              ...
              {imagePreview ? (
                <div className="relative border-2 border-border rounded-xl overflow-hidden group">
                  ...
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <Button type="button" variant="outline" size="sm" className="border-red-500 text-red-500 hover:bg-red-500 hover:text-white" onClick={() => { setImageFile(null); setImagePreview(existingImageUrl || null); }}>
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                </div>
